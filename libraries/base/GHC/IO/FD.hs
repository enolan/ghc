...
blockingReadRawBufferPtr loc fd buf off len
  = throwErrnoIfMinus1Retry loc $
        if fdIsSocket fd
           ...
           else fmap fromIntegral $ c_safe_read (fdFD fd) (buf `plusPtr` off) len
  ...
  = throwErrnoIfMinus1Retry loc $
        if fdIsSocket fd
           then c_safe_send  (fdFD fd) (buf `plusPtr` off) len 0
             ...
             r <- fmap fromIntegral $ c_safe_write (fdFD fd) (buf `plusPtr` off) len
             when (r == -1) c_maperrno
             return r
...
foreign import WINDOWS_CCONV safe "recv"
   c_safe_recv :: CInt -> Ptr Word8 -> CSize -> CInt{-flags-} -> IO CInt

foreign import WINDOWS_CCONV safe "send"
   c_safe_send :: CInt -> Ptr Word8 -> CSize -> CInt{-flags-} -> IO CInt
